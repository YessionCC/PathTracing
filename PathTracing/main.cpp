...
void scene3() {
	Object*obj10 = Object::load_obj("scenes/scene1/obj1-0.txt", true);
	Object*obj11 = Object::load_obj("scenes/scene1/obj1-1.txt", true);
	Object*obj12 = Object::load_obj("scenes/scene1/obj1-2.txt", true);
	obj10->color = Vec3(1, 1, 1);
	obj10->material = Material::SPECULAR;
	obj11->color = Vec3(0.1, 0.1, 1);
	obj11->material = Material::DIFFUSION;
	...
	obj12->material = Material::CERAMIC;
	obj12->ceramic_smooth = 0.02f;
	Object*deer = Object::load_obj("scenes/scene3/deer.obj", true);
	deer->std_scale(2);
	deer->inverse_y_axis();
	deer->rotate_arround_z(30);
	deer->move(Vec3(3, 3.4, 2));
	deer->color = Vec3(1, 1, 1);
	deer->material = Material::REFRACTION;
	Object*cube = _ins->load_obj("scenes/scene2/cube.txt", true);
	cube->move(Vec3(1, 1, 0.5));
...
void scene4() {
	...
	cube1->material = Material::EMISSION;
	Object*cube2 = _ins->load_obj("scenes/scene2/cube.txt", true);
	_ins->move_obj(cube2, Vec3(3.2, 3.4, 3));
	cube2->emission = Vec3(1, 1, 1);
	cube2->color = Vec3(1, 1, 1);
	cube2->material = Material::EMISSION;
	Object* dra = Object::load_obj("scenes/scene5/dragon.obj", true);
	dra->std_scale(5.4);
	dra->inverse_y_axis();
	dra->rotate_arround_z(-120);
	dra->move(Vec3(3.2, 3.4, 1));
	dra->material = Material::REFRACTION;
	dra->rfcn = 1.15f;
	dra->color = Vec3(1, 0.4, 0.3);
}
...
int main() {
	...
	int w = 1024, h = 768;
	Vec3 *cols = camera.render(w, h, 60, 32, 16);
	initgraph(w, h, 1);
